...
Select the folder containing the image(s) to be loaded. Generally, it is
...
typos are avoided. %(IO_FOLDER_CHOICE_HELP_TEXT)s

%(IO_WITH_METADATA_HELP_TEXT)s %(USING_METADATA_TAGS_REF)s For
instance, if you have a “Plate” metadata tag, and your single files are
...
-  Using the metadata associated with an image set in **LoadImages** or
   **LoadData**. This is especially useful if you want your output given
   ...
   %(USING_METADATA_TAGS_REF)s%(USING_METADATA_HELP_REF)s.
...
-  *%(IO_OBJECTS)s:* Use this option if the input image is a label
   ...
   first. See **IdentifyPrimaryObjects** for more details.""" % globals()))
...
Select *%(YES)s* if you want to save an image of the outlines of the
