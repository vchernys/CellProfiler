...
<b>CorrectIlluminationCalculate</b>, to an image in order to correct for uneven illumination (uneven shading).
<hr>
This module applies a previously created illumination correction function, either loaded by <b>LoadSingleImage</b> or
created by <b>CorrectIlluminationCalculate</b>. This module corrects each image in the pipeline using the function
specified. See also <b>CorrectIlluminationCalculate</b>.
"""
...
import correctilluminationcalculate
...
class CorrectIlluminationApply(cellprofiler.module.Module):
    ...
    def add_image(self, can_delete=True):
        ...
        illum_correct_function_image_name = cellprofiler.setting.ImageNameSubscriber(
            ...
            cellprofiler.setting.NONE,
            doc="""
            Select the illumination correction function image that will be used to carry out the correction.
            This image is usually produced by another module or loaded as a .mat format image using the
            <b>Images</b> module or <b>LoadSingleImage</b>.
            """
        )
        ...
        divide_or_subtract = cellprofiler.setting.Choice(
            ...
            doc="""
            This choice depends on how the illumination function was calculated and on your physical model of
            the way illumination variation affects the background of images relative to the objects in images;
            it is also somewhat empirical.
            <ul>
                <li><i>{DOS_SUBTRACT}:</i> Use this option if the background signal is significant relative to
                the real signal coming from the cells. If you created the illumination correction function
                using <i>{IC_BACKGROUND}</i>, then you will want to choose <i>{DOS_SUBTRACT}</i> here.</li>
                <li><i>{DOS_DIVIDE}:</i> Choose this option if the signal to background ratio is high (the
                cells are stained very strongly). If you created the illumination correction function using
                <i>{IC_REGULAR}</i>, then you will want to choose <i>{DOS_DIVIDE}</i> here.</li>
            </ul>
            """.format(**{
                "DOS_SUBTRACT": DOS_SUBTRACT,
                "IC_BACKGROUND": correctilluminationcalculate.IC_BACKGROUND,
                "DOS_DIVIDE": DOS_DIVIDE,
                "IC_REGULAR": correctilluminationcalculate.IC_REGULAR
            })
        )
