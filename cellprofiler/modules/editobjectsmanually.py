...
<hr>
The interface will show the image that you selected as the guiding image, overlaid with colored outlines of the
selected objects (or filled objects if you choose). This module allows you to remove or edit specific objects by
pointing and clicking to select objects for removal or editing. Once editing is complete, the module displays the
objects as originally identified (left) and the objects that remain after this module (right). More detailed Help is
provided in the editing window via the '?' button. The pipeline pauses once per processed image when it reaches this
module. You must press the <i>Done</i> button to accept the selected objects and continue the pipeline.
...
</ul>See also <b>FilterObjects</b>, <b>MaskObject</b>, <b>OverlayOutlines</b>, <b>ConvertToImage</b>.
"""
...
class EditObjectsManually(identify.Identify):
    ...
    def create_settings(self):
        self.object_name = cellprofiler.setting.ObjectNameSubscriber(
            ...
            doc="""
            Choose a set of previously identified objects for editing, such as those produced by one of the
        ...
        self.filtered_objects = cellprofiler.setting.ObjectNameProvider(
            ...
            "EditedObjects",
            doc="""
            Enter the name for the objects that remain after editing. These objects will be available for use
        ...
        self.allow_overlap = cellprofiler.setting.Binary(
            ...
            False,
            doc="""
            <b>EditObjectsManually</b> can allow you to edit an object so that it overlaps another or it can
            prevent you from overlapping one object with another. Objects such as worms or the neurites of
            neurons may cross each other and might need to be edited with overlapping allowed, whereas a
            monolayer of cells might be best edited with overlapping off.<br>
            Select <i>{YES}</i> to allow overlaps or select <i>{NO}</i> to prevent them.
            """.format(**{
        ...
        self.wants_outlines = cellprofiler.setting.Binary(
            ...
            False,
            doc="""
            Select <i>{YES}</i> if you want to keep images of the outlines of the objects that remain after
            editing. This image can be saved by downstream modules or overlayed on other images using the
            <b>OverlayOutlines</b> module.
            """.format(**{
                "YES": cellprofiler.setting.YES
            })
        )
        ...
        self.renumber_choice = cellprofiler.setting.Choice(
            ...
            [R_RENUMBER, R_RETAIN],
            doc="""
            Choose how to number the objects that remain after editing, which controls how edited objects are
            associated with their predecessors:
            <ul>
                <li><i>{R_RENUMBER}:</i> The module will number the objects that remain using consecutive
                numbers. This is a good choice if you do not plan to use measurements from the original objects
                and you only want to use the edited objects in downstream modules; the objects that remain
                after editing will not have gaps in numbering where removed objects are missing.</li>
                <li><i>{R_RETAIN}:</i> This option will retain each object's original number so that the edited
                object's number matches its original number. This allows any measurements you make from the
                edited objects to be directly aligned with measurements you might have made of the original,
                unedited objects (or objects directly associated with them).</li>
            </ul>
            """.format(**{
        ...
        self.wants_image_display = cellprofiler.setting.Binary(
            ...
            Select <i>{NO}</i> if you do not want a guide image while editing
            """.format(**{
                "YES": cellprofiler.setting.YES,
                "NO": cellprofiler.setting.NO
            })
        ...
        self.image_name = cellprofiler.setting.ImageNameSubscriber(
            ...
            This is the image that will appear when editing objects. Choose an image supplied by a previous
            module.
            """
        )
    ...
    def run(self, workspace):
        ...
        assert isinstance(orig_objects, cellprofiler.object.Objects)
        ...
        #
        filtered_objects = cellprofiler.object.Objects()
        ...
        m.add_measurement(filtered_objects_name,
                          identify.FF_PARENT % orig_objects_name,
                          parents)
        m.add_measurement(orig_objects_name,
                          identify.FF_CHILDREN_COUNT % filtered_objects_name,
                          child_count)
        #
        # The object count
        #
        identify.add_object_count_measurements(m, filtered_objects_name,
                                               object_count)
        #
        # The object locations
        #
        identify.add_object_location_measurements_ijv(m, filtered_objects_name, ijv)
        #
    ...
    def get_measurement_columns(self, pipeline):
        ...
        columns = identify.get_object_measurement_columns(filtered_image_name)
        columns += [(orig_image_name,
                     identify.FF_CHILDREN_COUNT % filtered_image_name,
                     cellprofiler.measurement.COLTYPE_INTEGER),
                    (filtered_image_name,
                     identify.FF_PARENT % orig_image_name,
                     cellprofiler.measurement.COLTYPE_INTEGER)]
        return columns
