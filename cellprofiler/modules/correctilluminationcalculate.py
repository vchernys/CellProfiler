...
<hr>
This module calculates an illumination function that can either be saved to the hard drive for later use or immediately
applied to images later in the pipeline. This function will correct for the uneven illumination in images. If saving,
select <i>.mat</i> format in <b>SaveImages</b>. Use the <b>CorrectIlluminationApply</b> module to apply the function to
the image to be corrected. Illumination correction is a challenge to do properly; please see the <a href=
"http://www.cellprofiler.org/examples.html">examples</a> and <a href=
"http://cellprofiler.org/tutorials.html">tutorials</a> pages on the CellProfiler website for further advice. See also
<b>CorrectIlluminationApply</b>, <b>EnhanceOrSuppressFeatures</b>.
"""
...
class CorrectIlluminationCalculate(cellprofiler.module.Module):
    ...
    def create_settings(self):
        ...
        self.illumination_image_name = cellprofiler.setting.ImageNameProvider(
            ...
            provided_attributes={
                cellprofiler.setting.AGGREGATE_IMAGE_ATTRIBUTE: True,
        ...
        self.intensity_choice = cellprofiler.setting.Choice(
            ...
            IC_REGULAR,
            doc="""
            Choose which method you want to use to calculate the illumination function. You may chose from the
            following options:
            <ul>
                <li><i>{IC_REGULAR}:</i> If you have objects that are evenly dispersed across your image(s) and
                cover most of the image, the <i>Regular</i> method might be appropriate. Regular intensities
                makes the illumination function based on the intensity at each pixel of the image (or group of
                images if you are in <i>{EA_ALL}</i> mode) and is most often rescaled (see below) and applied
                by division using <b>CorrectIlluminationApply.</b> Note that if you are in <i>{EA_EACH}</i>
                mode or using a small set of images with few objects, there will be regions in the average
                image that contain no objects and smoothing by median filtering is unlikely to work well.
                <i>Note:</i> it does not make sense to choose (<i>{IC_REGULAR} + {SM_NONE} + {EA_EACH}</i>)
                because the illumination function would be identical to the original image and applying it will
                yield a blank image. You either need to smooth each image, or you need to use <i>{EA_ALL}</i>
                images.</li>
                <li><i>{IC_BACKGROUND}:</i> If you think that the background (dim points) between objects show
                the same pattern of illumination as your objects of interest, you can choose the
                <i>{IC_BACKGROUND}</i> method. Background intensities finds the minimum pixel intensities in
                blocks across the image (or group of images if you are in <i>{EA_ALL}</i> mode) and is most
                often applied by subtraction using the <b>CorrectIlluminationApply</b> module. <i>Note:</i> if
                you will be using the <i>{DOS_SUBTRACT}</i> option in the <b>CorrectIlluminationApply</b>
                module, you almost certainly do not want to rescale the illumination function.</li>
            </ul>
            <p>Please note that if a mask was applied to the input image, the pixels outside of the mask will
            be excluded from consideration. This is useful, for instance, in cases where you have masked out
            the well edge in an image from a multi-well plate; the dark well edge would distort the
            illumination correction function along the interior well edge. Masking the image beforehand solves
            this problem.</p>
            """.format(**{
        ...
        self.dilate_objects = cellprofiler.setting.Binary(
            ...
            For some applications, the incoming images are binary and each object should be dilated with a
            Gaussian filter in the final averaged (projection) image. This is for a sophisticated method of
            illumination correction where model objects are produced. Select <i>{YES}</i> to dilate objects for
            this approach.
            """.format(**{
        ...
        self.object_dilation_radius = cellprofiler.setting.Integer(
            ...
            0,
            doc="""
            <i>(Used only if the "{IC_REGULAR}" method and dilation is selected)</i><br>
        ...
        self.block_size = cellprofiler.setting.Integer(
            ...
            1,
            doc="""
            <i>(Used only if "{IC_BACKGROUND}" is selected)</i><br>
            The block size should be large enough that every square block of pixels is likely to contain some
            background pixels, where no objects are located.
            """.format(**{
                "IC_BACKGROUND": IC_BACKGROUND
            })
        )
        ...
        self.rescale_option = cellprofiler.setting.Choice(
            ...
            doc="""
            The illumination function can be rescaled so that the pixel intensities are all equal to or greater
            than 1. You have the following options:
            <ul>
                <li><i>{YES}:</i> Rescaling is recommended if you plan to use the <i>{IC_REGULAR}</i> method
                (and hence, the <i>{DOS_DIVIDE}</i> option in <b>CorrectIlluminationApply</b>) so that the
                corrected images are in the range 0 to 1.</li>
                <li><i>{NO}:</i> Rescaling is not recommended if you plan to use the <i>{IC_BACKGROUND}</i>
                method, which is paired with the <i>{DOS_SUBTRACT}</i> option in
                <b>CorrectIlluminationApply</b>. Note that as a result of the illumination function being
                rescaled from 1 to infinity, the rescaling of each image might be dramatic if there is
                substantial variation across the field of view, causing the corrected images to be very
                dark.</li>
                <li>{RE_MEDIAN}<i>:</i> This option chooses the median value in the image to rescale so that
                division increases some values and decreases others.</li>
            </ul>
            """.format(**{
                "YES": cellprofiler.setting.YES,
                "IC_REGULAR": IC_REGULAR,
                "DOS_DIVIDE": DOS_DIVIDE,
        ...
        self.each_or_all = cellprofiler.setting.Choice(
            ...
            illumination function using just the current image or you can calculate the illumination function
            using all of the images in each group. The illumination function can be calculated in one of the
            three ways:
            <ul>
                <li><i>{EA_EACH}:</i> Calculate an illumination function for each image individually.</li>
                <li><i>{EA_ALL_FIRST}:</i> Calculate an illumination function based on all of the images in a
                group, performing the calculation before proceeding to the next module. This means that the
                illumination function will be created in the first cycle (making the first cycle longer than
                subsequent cycles), and lets you use the function in a subsequent
                <b>CorrectIllumination_Apply</b> module in the same pipeline, but also means that you will not
                have the ability to filter out images (e.g., by using <b>FlagImage</b>). The input images need
                to be assembled using the <b>Input</b> modules; using images produced by other modules will
                yield an error.</li>
                <li><i>{EA_ALL_ACROSS}:</i> Calculate an illumination function across all cycles in each group.
                This option takes any image as input; however, the illumination function will not be completed
                until the end of the last cycle in the group. You can use <b>SaveImages</b> to save the
                illumination function after the last cycle in the group and then use the resulting image in
                another pipeline. The option is useful if you want to exclude images that are filtered by a
                prior <b>FlagImage</b> module.</li>
            </ul>
            """.format(**{
                "EA_EACH": EA_EACH,
                "EA_ALL_FIRST": EA_ALL_FIRST,
                "EA_ALL_ACROSS": EA_ALL_ACROSS
            })
        )
        ...
        self.smoothing_method = cellprofiler.setting.Choice(
            ...
            doc="""
            If requested, the resulting image is smoothed. See the <b>EnhanceOrSuppressFeatures</b> module help
            for more details. If you are using <i>Each</i> mode, this is almost certainly necessary. If you
            have few objects in each image or a small image set, you may want to smooth.
            <p>You should smooth to the point where the illumination function resembles a believable pattern.
            For example, if you are trying to correct a lamp illumination problem, apply smoothing until you
            obtain a fairly smooth pattern without sharp bright or dim regions. Note that smoothing is a
            time-consuming process, but some methods are faster than others.</p>
            <ul>
                <li><i>{SM_FIT_POLYNOMIAL}:</i> This methdod is fastest but does not allow a very tight fit
                compared to the slower median and Gaussian filtering methods.</li>
                <li>{SM_MEDIAN_FILTER}, <i>{SM_GAUSSIAN_FILTER}:</i> Use a median or Gaussian filter,
                respectively. We typically recommend <i>{SM_MEDIAN_FILTER}</i> vs. <i>{SM_GAUSSIAN_FILTER}</i>
                because the median is less sensitive to outliers, although the results are also slightly less
                smooth and the fact that images are in the range of 0 to 1 means that outliers typically will
                not dominate too strongly anyway.</li>
                <li><i>{SM_TO_AVERAGE}:</i> A less commonly used option is to completely smooth the entire
                image, which will create a flat, smooth image where every pixel of the image is the average of
                what the illumination function would otherwise have been.</li>
                <li><i>{SM_SPLINES}:</i> This method (<i>Lindblad and Bengtsson, 2001</i>) fits a grid of cubic
                splines to the background while excluding foreground pixels from the calculation. It operates
                iteratively, classifying pixels as background, computing a best fit spline to this background
                and then reclassifying pixels as background until the spline converges on its final value.</li>
                <li>
                    <i>{SM_CONVEX_HULL}:</i> This method algorithm proceeds as follows:
                    <ul>
                        <li>Choose 256 evenly-spaced intensity levels between the minimum and maximum intensity
                        for the image</li>
                        <li>Set the intensity of the output image to the minimum intensity of the input
                        image</li>
                        <li>Iterate over the intensity levels, from lowest to highest
                            <ul>
                                <li>For a given intensity, find all pixels with equal or higher
                                intensities</li>
                                <li>Find the convex hull that encloses those pixels</li>
                                <li>Set the intensity of the output image within the convex hull to the current
                                intensity</li>
                            </ul>
                        </li>
                    </ul>The Convex Hull method can be used on an image whose objects are darker than their
                    background and whose illumination intensity decreases monotonically from the brightest
                    point.
                </li>
            </ul><b>References</b>
            <ul>
                <li>J Lindblad and E Bengtsson (2001) "A comparison of methods for estimation of intensity
                nonuniformities in 2D and 3D microscope images of fluorescence stained cells.", Proceedings of
                the 12th Scandinavian Conference on Image Analysis (SCIA), pp. 264-271</li>
            </ul>
            """.format(**{
                "SM_FIT_POLYNOMIAL": SM_FIT_POLYNOMIAL,
                "SM_MEDIAN_FILTER": SM_MEDIAN_FILTER,
                "SM_GAUSSIAN_FILTER": SM_GAUSSIAN_FILTER,
                "SM_TO_AVERAGE": SM_TO_AVERAGE,
                "SM_SPLINES": SM_SPLINES,
                "SM_CONVEX_HULL": SM_CONVEX_HULL
            })
        )
        ...
        self.automatic_object_width = cellprofiler.setting.Choice(
            ...
            <ul>
                <li><i>{FI_AUTOMATIC}:</i> The size is computed as 1/40 the size of the image or 30 pixels,
                whichever is smaller.</li>
                <li><i>{FI_OBJECT_SIZE}:</i> The size is obtained relative to the width of artifacts to be
                smoothed.</li>
                <li><i>{FI_MANUALLY}:</i> Use a manually entered value.</li>
            </ul>
            """.format(**{
                "FI_AUTOMATIC": FI_AUTOMATIC,
                "FI_OBJECT_SIZE": FI_OBJECT_SIZE,
                "FI_MANUALLY": FI_MANUALLY
            })
        )
        ...
        self.object_width = cellprofiler.setting.Integer(
            ...
            10,
            doc="""
            <i>(Used only if {FI_AUTOMATIC} is selected for smoothing filter size calculation)</i><br>
        ...
        self.size_of_smoothing_filter = cellprofiler.setting.Integer(
            ...
            10,
            doc="""
            <i>(Used only if {FI_MANUALLY} is selected for smoothing filter size calculation)</i><br>
        ...
        self.save_average_image = cellprofiler.setting.Binary(
            ...
            False,
            doc="""
            The averaged image is the illumination function prior to dilation or smoothing. It is an image
            produced during the calculations, not typically needed for downstream modules. It can be helpful to
            retain it in case you wish to try several different smoothing methods without taking the time to
            recalculate the averaged image each time.
            <p>Select <i>{YES}</i> to retain this averaged image. Use the <b>SaveImages</b> module to save it
        ...
        self.save_dilated_image = cellprofiler.setting.Binary(
            ...
            False,
            doc="""
            The dilated image is the illumination function after dilation but prior to smoothing. It is an
            image produced during the calculations, and is not typically needed for downstream modules.
            <p>Select <i>{YES}</i> to retain this dilated image. Use the <b>SaveImages</b> module to save it to
            your hard drive.</p>
            """.format(**{
                "YES": cellprofiler.setting.YES
            })
        ...
        self.automatic_splines = cellprofiler.setting.Binary(
            ...
            True,
            doc="""
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method)</i><br>
            Select <i>{YES}</i> to automatically calculate the parameters for spline fitting.
            <p>Select <i>{NO}</i> to specify the background mode, background threshold, scale, maximum number
            of iterations and convergence.</p>
            """.format(**{
                "SM_SPLINES": SM_SPLINES,
                "YES": cellprofiler.setting.YES,
                "NO": cellprofiler.setting.NO
        ...
        self.spline_bg_mode = cellprofiler.setting.Choice(
            "Background mode",
            [centrosome.bg_compensate.MODE_AUTO, centrosome.bg_compensate.MODE_DARK, centrosome.bg_compensate.MODE_BRIGHT, centrosome.bg_compensate.MODE_GRAY],
            doc="""
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method and spline parameters are not
            calculated automatically)</i><br>
            This setting determines which pixels are background and which are foreground.<br>
            <ul>
                <li><i>{MODE_AUTO}</i>: Determine the mode from the image. This will set the mode to
                {MODE_DARK} if most of the pixels are dark, {MODE_BRIGHT} if most of the pixels are bright and
                {MODE_GRAY} if there are relatively few dark and light pixels relative to the number of
                mid-level pixels</li>
                <li><i>{MODE_DARK}</i>: Fit the spline to the darkest pixels in the image, excluding brighter
                pixels from consideration. This may be appropriate for a fluorescent image.</li>
                <li><i>{MODE_BRIGHT}</i>: Fit the spline to the lightest pixels in the image, excluding the
                darker pixels. This may be appropriate for a histologically stained image.</li>
                <li><i>{MODE_GRAY}</i>: Fit the spline to mid-range pixels, excluding both dark and light
                pixels. This may be appropriate for a brightfield image where the objects of interest have
                light and dark features.</li>
            </ul>
            """.format(**{
                "SM_SPLINES": SM_SPLINES,
                "MODE_AUTO": centrosome.bg_compensate.MODE_AUTO,
                "MODE_DARK": centrosome.bg_compensate.MODE_DARK,
                "MODE_BRIGHT": centrosome.bg_compensate.MODE_BRIGHT,
                "MODE_GRAY": centrosome.bg_compensate.MODE_GRAY
            })
        )
        ...
        self.spline_threshold = cellprofiler.setting.Float(
            ...
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method and spline parameters are not
            calculated automatically)</i><br>
            This setting determines the cutoff used when excluding foreground pixels from consideration. On
            each iteration, the method computes the standard deviation of background pixels from the computed
            background. The number entered in this setting is the number of standard deviations a pixel can be
            from the computed background on the last pass if it is to be considered as background during the
            next pass.
            <p>You should enter a higher number to converge stabily and slowly on a final background and a
            lower number to converge more rapidly, but with lower stability. The default for this parameter is
            two standard deviations; this will provide a fairly stable background estimate.</p>
            """.format(**{
                "SM_SPLINES": SM_SPLINES
            })
        )
        ...
        self.spline_points = cellprofiler.setting.Integer(
            ...
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method and spline parameters are not
            calculated automatically)</i><br>
            This is the number of control points for the spline. A value of 5 results in a 5x5 grid of splines
            across the image and is the value suggested by the method's authors. A lower value will give you a
            more stable background while a higher one will fit variations in the background more closely and
        ...
        self.spline_rescale = cellprofiler.setting.Float(
            ...
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method and spline parameters are not
            calculated automatically)</i><br>
            This setting controls how the image is resampled to make a smaller image. Resampling will speed up
            processing, but may degrade performance if the resampling factor is larger than the diameter of
            foreground objects. The image will be downsampled by the factor you enter. For instance, a 500x600
            image will be downsampled into a 250x300 image if a factor of 2 is entered.
            """.format(**{
                "SM_SPLINES": SM_SPLINES
            })
        )
        ...
        self.spline_maximum_iterations = cellprofiler.setting.Integer(
            ...
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method and spline parameters are not
            calculated automatically)</i><br>
            This setting determines the maximum number of iterations of the algorithm to be performed. The
            algorithm will perform fewer iterations if it converges.
            """.format(**{
                "SM_SPLINES": SM_SPLINES
            })
        )
        ...
        self.spline_convergence = cellprofiler.setting.Float(
            ...
            <i>(Used only if {SM_SPLINES} are selected for the smoothing method and spline parameters are not
            calculated automatically)</i><br>
            This setting determines the convergence criterion. The software sets the convergence criterion to
            the number entered here times the signal intensity; the convergence you enter is the fraction of
            the signal intensity that indicates convergence. The algorithm derives a standard deviation of the
            background pixels from the calculated background on each iteration. The algorithm terminates when
            the difference between the standard deviation for the current iteration and the previous iteration
            is less than the convergence criterion.
            <p>Enter a smaller number for the convergence to calculate a more accurate background. Enter a
            larger number to calculate the background using fewer iterations, but less accuracy.</p>
            """.format(**{
                "SM_SPLINES": SM_SPLINES
            })
        )
    ...
    def apply_dilation(self, image, orig_image=None):
        ...
        if self.dilate_objects.value:
            ...
            return cellprofiler.image.Image(dilated_pixels, parent_image=orig_image)
        else:
            return image
        ...
        if (self.intensity_choice == IC_REGULAR or
            ...
            if orig_image.has_mask:
                if pixels.ndim == 2:
                    ...
                    pixels[~ orig_image.mask, :] = 0
                avg_image = cellprofiler.image.Image(pixels, parent_image=orig_image)
            else:
            ...
            avg_image = cellprofiler.image.Image(min_block, parent_image=orig_image)
        return avg_image
    ...
    def apply_smoothing(self, image, orig_image=None):
        ...
        output_image = cellprofiler.image.Image(output_pixels, parent_image=orig_image)
        return output_image
